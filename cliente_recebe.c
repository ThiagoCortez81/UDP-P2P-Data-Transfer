 ...
 * Para compilar:
 * gcc -o clienteRecebeUDP cliente_recebe.c -lws2_32 -lwsock32 -L $MingGW\lib
 */
#include <stdio.h>
#include <winsock2.h>
#include <sys/types.h>
    ...
    if (winSockInit(&data, &sock_cliente))
    {
        ...
        while (1)
        {
            ...
            if (buffer[0] != '\0') // Se o buffer for diferente de vazio
            {
                ...
                else if (strcmp(buffer, "0Erro-0002") == 0) // Se o arquivo não foi encontrado no cliente destino
                {
        ...
        while (1)
        {
            ...
            if (buffer[0] != '\0') // Se o buffer for diferente de vazio
            {
                ...
                {
                    checksum(buffer, size_receiving_message, ch); // Realiza o checksum do pacote e salva na variável 'ch'
                    if (ch[0] == buffer[2] && ch[1] == buffer[3]) // Verifica se os checksums enviados, batem com o aqui calculado, após o recebimento
                    {
